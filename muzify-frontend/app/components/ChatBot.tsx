...
const ChatBot = ({ isAdmin }: ChatBotProps) => {
  ...
  <div className="bg-slate-800 w-full p-3 h-full flex flex-col rounded-2xl">
      ...
      {messages ? (
        messages.map((msg, index) => (
          ...
          <div className="flex flex-col justify-start w-full items-start px-3">
            <p className="text-[10px] font-semibold text-gray-400">{msg.sender}</p>
...
<div ref={messagesEndRef} />
    ...
    <div className="w-full h-fit flex gap-2 sm:gap-4 items-center p-2 sm:p-4">
      <input
        ...
        className="bg-transparent border-2 border-black text-white w-full h-10 sm:h-12 rounded-xl px-4 outline-none text-sm sm:text-base placeholder:text-gray-600"
      />
      <button
        onClick={handleSend}
