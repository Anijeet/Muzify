...
const Queue = () => {
  ...
  // @ts-ignore
    ...
    return (
        <div className="bg-slate-800 flex flex-col md:gap-2 gap-4 pb-4 pt-4 overflow-x-auto scrolll px-6 rounded-2xl w-full h-full">
            <div className="w-full flex justify-between items-center">
                        <h1 className="text-xl text-white font-roboto font-semibold">Next in Row</h1> 
                        {/* {userId ? userId : "no user id"} */}
                        <button onClick={()=>{router.push('/room')}} className="bg-red-600 text-white font-serif p-1 rounded-lg">Leave Room</button>
            </div>
        ...
        queue.map((item: { thumbnail: string | undefined;hasLiked: boolean; title: any; upvoteCount: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; streamId: unknown; }, index: Key | null | undefined) => (
          <div
            key={index}
            className="flex flex-col min-w-[33vw] max-w-[36vw] overflow-hidden shrink-0 md:min-w-0 md:w-[13vw] md:h-[24vh] border h-full rounded-2xl text-white hover:bg-gray-600 p-2 items-center justify-between"
          >
            <div className="flex flex-col items-center gap-2 h-full">
              <div className="min-w-[10vw] min-h-[10vh] rounded-xl overflow-hidden">
                <img
                  src={item.thumbnail}
                  alt="Preview Image"
        ...
        <div className="w-full h-full text-white ">No song in queue at the moment</div>
      )}
            </div>
        </div>
    )
